...
const programVisuals = [
  ['program-face-symbol-v3.webp', 1672, 941],
  ['program-body-organic-v10.webp', 1516, 1037],
  ['program-private-valley-v3.webp', 1672, 941],
] as const;
...
export default function Home() {
  ...
  return <main data-reveal-root><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/><RevealMotion/>
    <section className="hero">
      <div className="heroVisual" aria-label="파장 위에 배치된 남성 턱선 조각상"><img className="heroCutout" src="./hero-david-cutout-v2.webp" width="1672" height="941" fetchPriority="high" decoding="async" alt="청색 레이저 선이 턱선을 따라 흐르는 남성 조각상"/></div>
      <div className="heroInner">
        <div className="heroBrand"><img src="./jj-mark.webp" width="2160" height="2160" decoding="async" alt="JJ비뇨기과"/><span>UROLOGY</span></div>
    ...
    <section className="pricing wrap" id="price"><header className="sectionTitle split"><div><p className="label blue">비용 안내</p><h2>시술 부위별<br/>비용 안내</h2></div><p>표기 금액 단위는 만원입니다.<br/>개인의 상태와 실제 시술 범위에 따라 달라질 수 있습니다.</p></header><div className="priceTable">{priceGroups.map(([group, items], i) => <section key={group}><div className="priceVisual"><img src={`./${priceVisuals[i]}`} width="1254" height="1254" loading="lazy" decoding="async" alt={`${group} 제모 부위를 표현한 이미지`}/><h3>{group}</h3></div><div>{items.map(([name, price]) => <p key={name}><span>{name}</span><strong>{price}<small>만원</small></strong></p>)}</div></section>)}</div><a className="priceCta" href="#consultation">내게 맞는 부위 상담하기 <span>→</span></a></section>

    <section className="careGuide"><div className="wrap careColumns"><article><div className="careContent"><Icon name="before"/><h3>시술 전 확인사항</h3><p>시술 1~2일 전 면도기로 가볍게 면도해 주세요. 털을 뽑거나 왁싱하면 레이저가 목표로 하는 모근이 사라질 수 있습니다. 최소 2주간 선탠과 과도한 야외 활동도 피해주세요.</p></div></article><article><div className="careContent"><Icon name="after"/><h3>시술 후 관리</h3><p>당일 미온수 샤워는 가능하며 3~5일간 사우나·찜질방·격한 운동을 피해주세요. 노출 부위에는 자외선 차단제를 꼼꼼히 사용하고 이상 반응이 지속되면 의료진에게 문의하세요.</p></div></article></div></section>
