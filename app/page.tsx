...
export default function Home() {
  return <main>
    ...
    <section className="strengths wrap"><header className="sectionTitle"><p className="label blue">남성 전담 프라이빗 케어</p><h2>망설였던 이유까지<br/>케어의 기준으로.</h2></header><div className="strengthList">{strengths.map(([n, icon, title, text]) => <article key={n}><span>{n}</span><Icon name={icon as IconName}/><h3>{title}</h3><p>{text}</p></article>)}</div></section>
